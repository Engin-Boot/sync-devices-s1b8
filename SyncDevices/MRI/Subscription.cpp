...
int subscribe() {
	...
	rc = MQTTAsync_createWithOptions(&client, url, sub_opts.clientid, MQTTCLIENT_PERSISTENCE_NONE, NULL, &create_opts);
	...
	rc = MQTTAsync_setCallbacks(client, client, NULL, messageArrived_sub, NULL);
	if (rc != MQTTASYNC_SUCCESS)
	{
		if (!sub_opts.quiet)
